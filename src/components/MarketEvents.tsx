...
function stripPublisherCredit(text: string) {
  return text
...
function fallbackBlocks(event: MarketEvent): BriefArticleBlock[] {
  ...
  const text = stripPublisherCredit(event.detail || event.summary);
  return text
    .split(/\n{2,}/)
    .map((part) => stripPublisherCredit(part))
    .filter(Boolean)
        ...
        <div className="flex items-start justify-between gap-4">
          <div className="min-w-0">
            <p className="text-xs font-semibold uppercase tracking-widest text-violet">
              {event.category || regionLabels[event.region]} · {event.impact}
            </p>
            <h2
    ...
    >
      <div className="space-y-5">
        <div className="grid gap-3 sm:grid-cols-3 lg:grid-cols-6">
          <Stat label="Region" value={regionLabels[event.region]} />
          <Stat label="Tone" value={event.impact} />
          <Stat label="Published" value={formatDate(event.date) || "—"} />
          <Stat label="Desk" value={event.category || regionLabels[event.region]} />
...
function EventCard({
  ...
  const teaser = stripPublisherCredit(event.summary);
...
export function MarketEvents({ events }: { events: MarketEvent[] }) {
  ...
  const openEvent =
    openIndex != null && slots[openIndex] && slots[openIndex].date
      ? slots[openIndex]
      : null;
  ...
  return (
    <div className="glass flex h-full flex-col rounded-2xl p-6">
      ...
      <p className="mt-1 text-sm text-ink-soft">
        Headlines from this session. Tap a card to open the article.
      </p>
