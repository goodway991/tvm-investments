...
export function AuthPage({ initialMode }: { initialMode: AuthMode }) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    const credentialEmail =
      mode === "login" && email.trim().toUpperCase() === "ADMIN"
        ...
        : email.trim();
    if (mode === "signup" && !isValidEmailAddress(credentialEmail)) {
      setError("Enter a valid email address we can send a verification code to.");
      return;
    }
    if (
      mode === "login" &&
      credentialEmail !== BOOTSTRAP_ADMIN_EMAIL &&
      !isValidEmailAddress(credentialEmail)
    ) {
    ...
    try {
      ...
      if (authError instanceof FirebaseError) {
        const messages: Record<string, string> = {
          ...
          "auth/weak-password":
            "Use a password with at least 12 characters, including a letter and a number.",
  ...
  async function resetPassword() {
    ...
    if (resetEmail !== BOOTSTRAP_ADMIN_EMAIL && !isValidEmailAddress(resetEmail)) {
    ...
    try {
      ...
      if (!response.ok) {
        setError(payload.error || "Unable to send a password reset email.");
        return;
      }
      setMessage(payload.message || "If that email is on file, we sent a reset link.");
    } catch {
